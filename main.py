...
import sys        #We will use this to exit the program
import pygame
...
SCREENWIDTH = 289
SCREENHEIGHT = 511
SCREEN = pygame.display.set_mode((SCREENWIDTH,SCREENHEIGHT))  #initialize a window or a screen for display
GROUNDY = SCREENHEIGHT * 0.8
GAME_SPRITES = {}
GAME_SOUNDS = {}
PLAYER = 'gallery/sprites/redbird.png'
...
def mainGame():
    ...
    while True:
        ...
        #Add a new pipe when the first pipe abt to cross the leftmost part of the screen
        if(0<upperPipes[0]['x']<5):
            newpipe = getRandomPipe()
            upperPipes.append(newpipe[0])
            lowerPipes.append(newpipe[1])

        #if pipe is out of the screen remove it just
...
def showGameOverScreen():
    ...
    '''
    # Display the Game Over sprite in the middle of the screen
    gameOverX = int((SCREENWIDTH - GAME_SPRITES['gameover'].get_width()) / 2)
    gameOverY = int(SCREENHEIGHT * 0.3)
    ...
    while True:
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                ...
                welcomeScreen()  # Restart game from the welcome screen
                return
...
def getRandomPipe():
    '''
    generate pos of 2 pipes for blitting on scrren (1 straight and 1 rotated)
    '''
    pipeHeight=GAME_SPRITES['pipe'][0].get_height()
...
if __name__=="__main__":
    ...
    FPSCLOCK = pygame.time.Clock() #it controls the fps of game
    pygame.display.set_caption("Flappy Bird By Souvik")
    ...
    GAME_SPRITES['base']=pygame.image.load('gallery/sprites/base_2.png').convert_alpha()
